...
useEffect に指定する関数は、戻り値として関数ポインタをクリーンアップ用関数として指定できる（関数ポインタとは言わないっぽい...。なんて言うのが適切なんだろ）。ここで指定した関数は、以下のタイミングで実行される。
...
b89bee6 は、開いたドロップダウンリストを表示しているとき、ウィンドウ内の任意の場所がクリックされたらドロップダウンリストを閉じるって修正。  
TODO
